...
router.post('/',(req,res)=>{
	...
	var geoip = await axios.get(`http://ip-api.com/json/ip`);
	var country = geoip.data.countryCode; 
	const sql="INSERT INTO contactos(nombre, email, comentario, fecha ,ip,country) VALUES (?,?,?,?,?,?)";
	const nuevos_mensajes=[req.body.nombre, req.body.email, req.body.comentario,fecha,ip,country];
	bd.run(sql, nuevos_mensajes, err =>{
	if (err){
		return console.error(err.message);
	}
